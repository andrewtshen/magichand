...
def main():
    ...
    while capture.isOpened():
        ...
        if is_hand_hist_created:
            ...
            motion(traverse_point,traverse_centroid)
...
def motion(traverse_point,traverse_centroid):
	...
	'''
	perform regression to check for scrolling: the idea is that if the person is scrolling, 
    both the centroid and farthest point are going to approximately move along a straight 
    line with similar slope
    '''
	
	#perform linear fit for the furthest points
	
	A_p = np.vstack([p_x, np.ones(len(p_x))]).T
	m_p, b_p = np.linalg.lstsq(A_p, p_y, rcond=None)[0]

	print("p coeff :",m_p,b_p)
	
	#perform linear fit for the centroid
	
	A_p = np.vstack([p_c, np.ones(len(p_c))]).T
	m_c, b_c = np.linalg.lstsq(A_c,c_y,rcond=None)[0]

	print("c coeff :",m_c,b_c)
